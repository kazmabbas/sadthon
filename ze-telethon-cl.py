...
∘ مطور ومبرمج البوت توفي - @zzsszzz
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', 'zekalb.py', 'sython.json', '__pycache__', 'sython-telethon-cl.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
@bot.on(events.CallbackQuery(pattern="button0"))
async def start(event):
    sender = await event.get_sender()
    if sender.id == DEVELOPER_ID:
        chat = await event.get_chat()
        ...
        await bot.send_message(chat, '''**اهلا بك عزيزي في بوت تحكم الخاص في سورس بدثون\n [قناة المطور](http://t.me/BDthon) \n [تواصل المطور](http://t.me/zzsszzz) \n\n اذا لكيت مشكله راسلني **''', buttons=buttons)
